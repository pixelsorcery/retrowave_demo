...
#if defined(_DEBUG)
    HRESULT hr = D3DCompile(source, strlen(source), NULL, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, entrypt, profile, D3DCOMPILE_ENABLE_STRICTNESS | D3DCOMPILE_DEBUG /*D3DCOMPILE_OPTIMIZATION_LEVEL3*/, 0,
        &code, &errors);
#else
    HRESULT hr = D3DCompile(source, strlen(source), NULL, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, entrypt, profile, D3DCOMPILE_OPTIMIZATION_LEVEL3, 0,
        &code, &errors);
#endif

    if (hr != S_OK && errors->GetBufferPointer())
    {
